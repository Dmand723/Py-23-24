...
class Game():
    def __init__(self,playenames):
        ...
        for i in range(len(playenames)):
            pPath = "assets//sprites//player//player"+str(i+1)
            player = Player(spawnPos[i],pPath,self,playenames[i])
    ...
    def update(self):
        ...
        self.allsprties.update(self.dt)
        self.curPlayer.curPlayerUpdate(self.dt)
