...
class Player(pg.sprite.Sprite):
    def __init__(self,spriteFolderName='',tag="Player",spriteAmount = 1,animatedSptrite = False,spriteColor = transparent):
        ...
        self.imgsize = ((TILE_SIZE[0])*2,(TILE_SIZE[1])*2)
        ...
        self.verticalMovement = True
        self.horizontalMovement = True
        self.boundType = 'wrap' #solid wrap bounce
        self.moveType = ['mouse','keyboard'] #mouse or keyboard if none then auto
        self.moveDirX = 1
        self.moveDirY = 1
    ...
    def move(self):
        ...
        if mousePos < self.curMousePos:#mouse going left 
            if not self.goingLeft:
                    self.image = pg.transform.flip(self.image,self.rect.x,0)
            self.goingLeft = True
            self.goingRight = False
        if mousePos > self.curMousePos:#mouse going right 
            if not self.goingRight:
                    self.image = pg.transform.flip(self.image,self.rect.x,0)
            self.goingLeft = False
            self.goingRight = True
